...
export const translatePlaceTypeSlug = (type, sourceLanguage = 'en') => {
  ...
  if (sourceLanguage === 'en') {
    ...
    return Object.keys(translations).find(k => translations[k] === type) || 'other'
  }
}
...
export const formatLivestockSuppliesLine = data => {
  ...
  let lines = Object.keys(flagsMap).filter(f => data[f] === '1').map(f => flagsMap[f]).join(', ')
  const drugsFlagsMap = {
  ...
  if (Object.keys(drugsFlagsMap).some(f => data[f] === '1')) {
    lines += ', Fármacos: ' + Object.keys(drugsFlagsMap).filter(f => data[f] === '1').map(f => drugsFlagsMap[f]).join(', ')
  }
  return lines
}
...
export const formatCertifications = certifications => {
  return certifications.filter(v => v[1] === 'S').map(v => v[0]).join(', ')
}
