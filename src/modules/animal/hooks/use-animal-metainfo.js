...
import { useLazyMetainfoAnimalQuery } from "../features/animal.rtk";

export default function useAnimalMetainfo() {
  const dispatch = useDispatch();
  const [groupCoords, setGroupCoords] = useState([]);
  const { searchTerm } = useSelector((state) => state.search);
  const [fetch, { isLoading, isFetching, data }] = useLazyMetainfoAnimalQuery();
