...
const PlaceSearchResultsList = () => {
  ...
  const [sortCriteria, setSortCriteria] = useState(
    SearchSortCriteria.Relevance
  );
  const { data, error, isLoading, isFetching } = useSearchForProviderQuery({
    search: searchTerm,
    type: searchType,
  });
  ...
  const { filteredData, count: filteredDataCount } = useMemo(() => {
    ...
    const filteredData = data.data.filter((d) => {
      let match =
        !generalFilter ||
        JSON.stringify(d).toLocaleLowerCase("es-ES").indexOf(generalFilter) >=
          0;
      ...
      match =
        match && (isEmpty(typesFilters) || typesFilters.indexOf(d.type) >= 0);
      return match;
    });
    switch (sortCriteria) {
    ...
    <div className="PlaceSearchResultsList">
      <div className="PlaceSearchResultsList__top">
        <h2 className="PlaceSearchResultsList__title">
          {filteredDataCount}{" "}
          {filteredDataCount === 1 ? "resultado" : "resultados"}
        </h2>
        <div
        ...
        <div className="PlaceSearchResultsList__sort_by_field_container">
          ...
          <select
            ...
            onChange={(e) =>
              setSortCriteria(SearchSortCriteria[e.target.value])
            }
            defaultValue={"Relevance"}
          >
            {Object.keys(SearchSortCriteria).map((criteria) => (
              <option key={`option-${criteria}`} value={criteria}>
      ...
      <div className="PlaceSearchResultsList__cards_container">
        {filteredData.slice(0, 100).map((result, i) => (
          <PlaceSearchResultCard key={`SearchResultCard-${i}`} data={result} />
        ))}
      </div>
    </div>
  );
