...
import { getIconByPlaceType, translatePlaceTypeSlug, formatPlaceType, formatHQ, formatLine, formatCategory, formatLivestockSuppliesLine, formatPoultry, formatCertifications, formatDate } from '../../helpers/places'
...
export const placeApi = createApi({
  ...
  endpoints: builder => ({
    place: builder.query({
      ...
      transformResponse: res => {
        ...
        const placeData = {
          ...
          padron: data.id || '',
          ruc: data.ruc || '',
          address: data.address_legal || data.address || data.address_real || '-',
    ...
    authorizations: builder.query({
      query: ({ id, spanishType }) => `/${translatePlaceTypeSlug(spanishType, 'es')}/auth/${id}`,
      transformResponse: res => {
        const results = res.data
        results.sort((r1, r2) => r1.end_date > r2.end_date ? -1 : 1)
        return results.map(result => {
          const {
            auth_id, auth_type, line, hq,
            address_real, contact_person, mobile_phone, end_date
          } = result
          return [
            { property: 'Fecha', value: end_date ? format(parseISO(end_date), 'dd-MM-yyyy') : '-' },
            { property: 'Código', value: auth_id + "-MINAGRI-SENASA-"+formatHQ(hq).replace('Dirección Ejecutiva ', '')},
            ...
            { property: 'Contacto', value: `${contact_person} ${mobile_phone ?? ''}` },
          ]
        })
      }
    }),
    csti: builder.query({
      query: ({ id, spanishType }) => `/${translatePlaceTypeSlug(spanishType, 'es')}/csti/${id}`,
      transformResponse: res => {
        const results = res.data
        results.sort((r1, r2) => r1.valid_start_date > r2.valid_start_date ? -1 : 1)
        return results.map(result => {
          const {
            csti_id, file_code, type, start_date, valid_start_date, valid_days,
            department_origin, province_origin, district_origin,
            contact_person_type, contact_person_dni, farm
          } = result
          return {
            properties: [
              { property: 'Fecha inicio CSTI', value: formatDate(valid_start_date) },
              ...
              { property: 'Días de validez', value: isNumber(valid_days) ? Number(valid_days) : '-' },
              { property: 'Departamento de origen', value: getNameOfLocation(department_origin, LocationType.Departamento) },
              { property: 'Provincia de origen', value: getNameOfLocation([department_origin, province_origin], LocationType.Provincia) },
              { property: 'Distrito de origen', value: getNameOfLocation([department_origin, province_origin, district_origin], LocationType.Distrito) },
              { property: 'DNI responsable', value: contact_person_dni },
              { property: 'Tipo responsable', value: contact_person_type },
              { property: 'Cód. productor', value: farm?.CODI_PROD_MOS ? farm.CODI_PROD_MOS : '-' },
              { property: 'Nombre productor', value: farm?.NOMB_PROD_MOS ? farm.NOMB_PROD_MOS : '-' },
            ],
            link: farm?.FARM_ID
          }
        })
    ...
    exportCertificate: builder.query({
      query: ({ id, spanishType }) => `/agricultural-exporter/certificates/${translatePlaceTypeSlug(spanishType, 'es')}/${id}`,
      transformResponse: res => {
        const results = res.data
        results.sort((r1, r2) => r1.inspection_date > r2.inspection_date ? -1 : 1)
        return results.map(result => {
          const {
            exporter_id, certificate_id, file_id, certificate_state, inspection_place, valid_start_date, importer,
            destination, transportation_mode, checkpoint,
            code_plant, plant_id, inspection_date, export_date, products
          } = result
          return {
            properties: [
              { property: 'Cód. certificado', value: certificate_id },
              { property: 'Estado', value: certificate_state },
              { property: 'Fecha inspección', value: formatDate(inspection_date) },
              { property: 'Planta', value: code_plant ? code_plant : '-' },
              { property: 'Punto de inspección', value: inspection_place ? inspection_place : '-' },
    ...
    exportOriginCertificate: builder.query({
      ...
      transformResponse: res => {
        ...
        return results.map(result => {
          const {
            application_id, certificate_id, camp_exportacion, area, start_date, products
          ...
          return {
            properties: [
              ...
              { property: 'Campaña', value: camp_exportacion ? camp_exportacion : '-' },
