...
import { formatCategory, formatHQ, getSlaughterhouseType, translatePlaceTypeSlug } from '../../helpers/places'
    ...
    searchForProvider: builder.query({
      ...
      transformResponse: res => {
        const data = res.data.map(item => {
          const [codDepartamento, codProvincia, codDistrito ] = item.ubigeo?.match(/.{2}/g) || ['100', '100', '100']
          return {
            name: item.name || '',
            ruc: item.ruc || '',
            padron: item.id || '',
            ...
            nameDepartamento: getNameOfLocation(codDepartamento, LocationType.Departamento),
            nameProvincia: getNameOfLocation([codDepartamento, codProvincia], LocationType.Provincia),
            nameDistrito: getNameOfLocation([codDepartamento, codProvincia, codDistrito], LocationType.Distrito),
    ...
    searchForAnimal: builder.query({
      ...
      transformResponse: res => {
        const transformedesponse = res.data.map(result => ({
          ...
          modifiedDate: result.fech_modi ? format(parseISO(result.fech_modi), `dd MMMM yyyy`, { locale: es }) : '-',
          company: result.nomb_prod_pro,
          sucursal: result.nomb_pred_pre,
          originCode: result.codi_pred_pre,
          highLow: result.high_low === 'A' ? 'Alta' : result.high_low === 'B' ? 'Baja' : result.high_low,
          nameDepartamento: getNameOfLocation(result.codi_depa_pro, LocationType.Departamento),
          nameProvincia: getNameOfLocation([result.codi_depa_pro, result.codi_prov_pro], LocationType.Provincia),
          nameDistrito: getNameOfLocation([result.codi_depa_pro, result.codi_prov_pro, result.codi_dist_pro], LocationType.Distrito),
        }))
    ...
    searchForFarm: builder.query({
      ...
      transformResponse: res => {
        const farms = res.data.map(result => {
          const {
            id, name, hq, department, district, province,
            state, geo_lat, geo_long, start_date, fech_modi,
            year, farmer_name, farmer_dni, farmer_ruc,
          } = result
          return {
            id,
